...
export async function onRequest(context) {
  ...
  if (
    ...
    url.pathname.startsWith("/api/create-recovery-landing") ||
    url.pathname.startsWith("/api/create-readingroom-landing") ||
...
function handleLogout(url) {
  return new Response(null, { status: 302, headers: { "Location": `${url.origin}/admin/login.html`, "Set-Cookie": "admin_session=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0" } });
}
...
async function verifySession(request, env) {
  ...
  const expected = await hmac(`${id}.${expires}`, env.ADMIN_SESSION_SECRET);
