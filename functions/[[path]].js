...
export async function onRequest(context) {
  ...
  if (group.landingKey && (pathname === "/" || pathname === "")) {
    const hubCases = await fetchCasesIndex(env);
    if (hubCases.length) {
      return new Response(renderDynamicHub(hubCases, group), {
        headers: {
...
async function fetchCasesIndex(env) {
  if (env.CASES) {
  ...
  return Array.isArray(parsed) ? parsed.map(buildHubIndexEntry) : [];
}

function buildHubIndexEntry(c) {
  return {
    slug: c.slug || "",
...
function renderDynamicHub(items, group) {
  const sorted = [...items]
    .filter((item) => item.slug)
    .sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || ""));
  const totalReports = sorted.reduce((sum, item) => sum + (Number(item.reports) || 0), 0);
  const key = group.landingKey || group.key;
  const suffix = hubSuffix(key);
  const title = hubTitle(key);
  const lead = hubLead(key);
  const rows = sorted.map((item, index) => {
    const name = normalizeCaseName(item.caseName || item.name || "");
    const displayTitle = suffix ? `${name} ${suffix}` : name;
    const href = `/${group.pathPrefix}/${encodeURIComponent(item.slug)}/`;
    return `<a href="${href}" class="case-row" data-title="${esc(name)}" data-slug="${esc(item.slug)}">
      <span class="case-no">${sorted.length - index}</span>
      <span class="case-title-wrap"><strong class="case-title">${esc(displayTitle)}</strong></span>
      <span class="case-status">${esc(hubStatus(key, item.slug))}</span>
...
<head>
  ...
  <link rel="canonical" href="${group.siteUrl}/">
  <meta name="robots" content="index, follow, max-image-preview:large, max-snippet:-1">
  <meta name="NaverBot" content="All">
  <meta name="Yeti" content="All">
  ${group.naverVerification ? `<meta name="naver-site-verification" content="${esc(group.naverVerification)}">` : ""}
  <meta property="og:type" content="website">
  <meta property="og:title" content="${esc(title)}">
  <meta property="og:description" content="${esc(lead)}">
  <meta property="og:url" content="${group.siteUrl}/">
  <meta property="og:site_name" content="${esc(group.siteName)}">
  <link rel="stylesheet" href="/assets/style.css">
</head>
<body class="${esc(group.bodyClass)}">
  <header class="site-header">
    <a class="brand" href="/" aria-label="home"><img src="/assets/logo.png" alt="logo"></a>
    <a class="header-call" href="tel:0269523695">02-6952-3695</a>
  </header>
  <main>
    <section class="hero">
      <p class="eyebrow">${esc(group.tone)}</p>
      <h1>${esc(title)}</h1>
      <p class="summary">${esc(lead)}</p>
    </section>
    <div class="page-shell">
      <section class="hub-stats-section">
        <div class="hub-stats">
          <div><span>${esc("\uB4F1\uB85D \uC0AC\uAC74")}</span><strong>${sorted.length.toLocaleString("ko-KR")}</strong></div>
          <div><span>${esc("\uB204\uC801 \uC811\uC218")}</span><strong>${totalReports.toLocaleString("ko-KR")}</strong></div>
        </div>
      </section>
      <div class="case-search-wrap">
        <input id="case-search" type="search" class="case-search" placeholder="${esc("\uC0AC\uAC74\uBA85 \uAC80\uC0C9")}" autocomplete="off">
        <button class="search-btn" type="button">${esc("\uAC80\uC0C9")}</button>
      </div>
      <section class="case-table-wrap" aria-label="${esc(title)}">
        <div class="case-table-title"><h2>${esc(group.tableTitle || title)}</h2><p class="case-table-lead">${esc(lead)}</p></div>
        <div class="case-table-header"><span>No.</span><span>${esc("\uC0AC\uAC74\uBA85")}</span><span>${esc("\uC0C1\uD0DC")}</span><span>${esc("\uB4F1\uB85D\uC77C")}</span><span>${esc("\uC870\uD68C\uC218")}</span></div>
        ${rows}
      </section>
    </div>
  </main>
  <script>
    document.getElementById('case-search')?.addEventListener('input', function(){
      ...
      document.querySelectorAll('.case-row').forEach(function(row){
        row.style.display=row.dataset.title.toLowerCase().indexOf(q)>=0?'grid':'none';
      });
    });
  </script>
</body>
...
function hubTitle(key) {
  return {
    la: "\uAE08\uC735\uD53C\uD574 \uD615\uC0AC \uC0AC\uAC74 \uC811\uC218 \uB9AC\uC2A4\uD2B8",
    lb: "\uD53C\uD574\uAE08 \uD68C\uC218 \uC804\uB7B5 \uC0AC\uAC74 \uB9AC\uC2A4\uD2B8",
    lc: "\uC2E4\uC81C \uD68C\uC218 \uC0AC\uB840 \uC544\uCE74\uC774\uBE0C",
    ld: "\uD53C\uD574 \uAD6C\uC870 AI \uBE0C\uB9AC\uD551 \uB9AC\uC2A4\uD2B8",
    le: "\uAE08\uC735\uD53C\uD574 \uC0AC\uAC74 \uD1B5\uD569 \uD5C8\uBE0C",
...
function hubLead(key) {
  return {
    la: "\uC0AC\uAE30 \uC758\uC2EC \uC0AC\uAC74\uC744 \uD615\uC0AC\uACE0\uC18C, \uC99D\uAC70 \uC815\uB9AC, \uCD94\uAC00 \uC785\uAE08 \uCC28\uB2E8 \uAD00\uC810\uC73C\uB85C \uBE60\uB974\uAC8C \uD655\uC778\uD569\uB2C8\uB2E4.",
    lb: "\uAC00\uC555\uB958, \uC190\uD574\uBC30\uC0C1, \uBD80\uB2F9\uC774\uB4DD\uBC18\uD658 \uAC00\uB2A5\uC131\uC744 \uC0AC\uAC74\uBCC4\uB85C \uBE44\uAD50\uD569\uB2C8\uB2E4.",
    lc: "\uC720\uC0AC \uC0AC\uAC74\uC758 \uC99D\uAC70 \uD65C\uC6A9\uACFC \uD68C\uC218 \uD750\uB984\uC744 \uC0AC\uB840 \uC911\uC2EC\uC73C\uB85C \uC815\uB9AC\uD569\uB2C8\uB2E4.",
    ld: "\uC0AC\uAC74 \uAC1C\uC694, \uD53C\uD574 \uD328\uD134, \uCD08\uAE30 \uB300\uC751 \uC21C\uC11C\uB97C \uC815\uBCF4\uD615 \uBB38\uC11C\uB85C \uC815\uB9AC\uD569\uB2C8\uB2E4.",
    le: "\uD615\uC0AC, \uBBFC\uC0AC, \uC0AC\uB840, AI \uBE0C\uB9AC\uD551 \uAD00\uC810\uC744 \uC0AC\uAC74\uBCC4\uB85C \uC5F0\uACB0\uD569\uB2C8\uB2E4.",
  }[key] || "\uC0AC\uAC74\uBCC4 \uB300\uC751 \uC815\uBCF4\uB97C \uC815\uB9AC\uD569\uB2C8\uB2E4.";
}

function hubSuffix(key) {
  return {
    la: "\uD615\uC0AC\uACE0\uC18C",
    lb: "\uD68C\uC218\uC804\uB7B5",
    lc: "\uD68C\uC218\uC0AC\uB840",
...
function hubStatus(key, seed) {
  if (key === "lc") return seededInt(`${seed}-rate`, 0, 100) < 25 ? "\uC804\uC561 \uD68C\uC218" : `${seededInt(`${seed}-pct`, 48, 98)}% \uD68C\uC218`;
  return {
    la: "\uD615\uC0AC \uC9C4\uD589\uC911",
    lb: "\uBBFC\uC0AC \uAC80\uD1A0\uC911",
    ld: "\uBD84\uC11D \uC644\uB8CC",
