...
function isRecoveryTopicCase(caseData = {}) {
  const text = `${caseData.caseName || ""} ${caseData.slug || ""} ${caseData.summary || ""}`;
  return /(지급정지|계좌정지|출금정지|이의제기|이의신청|채무부존재|계좌.*해제|해제.*계좌)/.test(text);
}

function selectRecoveryCarouselCases(caseData = {}, group = {}, relatedCases = []) {
...
function recoveryCarouselTitle(caseData = {}, group = {}) {
  const landing = caseData.landings?.[group.landingKey || group.key] || {};
  return landing.title || groupPageTitle(caseData.caseName || caseData.slug || "", "c", caseData);
}

function createRecoveryCarouselSchema(caseData, group, relatedCases, canonical) {
  const rows = selectRecoveryCarouselCases(caseData, group, relatedCases);
  if (rows.length < 4) return null;
  const bankName = recoveryBankName(caseData);
  return {
    "@type": "ItemList",
    "@id": `${canonical}#naver-carousel`,
    name: bankName ? `${bankName} 계좌 지급정지 해제 관련 안내` : "계좌 지급정지 해제 관련 안내",
    numberOfItems: rows.length,
    itemListOrder: "https://schema.org/ItemListOrderDescending",
    itemListElement: rows.map((item, index) => ({
      "@type": "ListItem",
      position: index + 1,
      name: recoveryCarouselTitle(item, group),
      image: caseOgPngImageUrl(item.slug, group.siteUrl),
      url: buildLandingUrl(group, item.slug),
    })),
  };
}

function createRecoveryCarouselSection(caseData, group, relatedCases = []) {
  const rows = selectRecoveryCarouselCases(caseData, group, relatedCases);
  if (rows.length < 4) return "";
  const bankName = recoveryBankName(caseData);
  const heading = bankName ? `${bankName} 계좌 지급정지 관련 안내 더 보기` : "은행 계좌 지급정지 관련 안내 더 보기";
  const cards = rows.map((item) => {
    const title = recoveryCarouselTitle(item, group);
    const url = buildLandingUrl(group, item.slug);
    const image = caseOgWebpImageUrl(item.slug, group.siteUrl);
    return `<li style="flex:0 0 210px;scroll-snap-align:start"><a href="${esc(url)}" style="display:block;color:inherit;text-decoration:none"><img src="${esc(image)}" alt="${esc(title)}" width="1200" height="630" loading="lazy" decoding="async" style="display:block;width:100%;height:118px;object-fit:cover;border-radius:12px"><strong style="display:block;margin-top:10px;font-size:15px;line-height:1.45">${esc(title)}</strong></a></li>`;
  }).join("");
  return `<section class="article-block recovery-related-carousel" aria-labelledby="recovery-carousel-title"><p class="section-kicker">RELATED GUIDE</p><h2 id="recovery-carousel-title">${esc(heading)}</h2><ul style="display:flex;gap:14px;overflow-x:auto;scroll-snap-type:x mandatory;list-style:none;margin:0;padding:4px 0 12px">${cards}</ul></section>`;
}
...
function createRecoveryCaseLandingContent(landing, group, caseData, relatedCases = []) {
  const rawCaseName = caseData.caseName || "";
  ...
  return [
    ...
    createRecoveryCarouselSection(caseData, group, relatedCases),
...
function createLandingContent(landing, group, caseData, relatedCases = []) {
  ...
  if (templateContentKey === "c") {
    return createRecoveryCaseLandingContent(landing, group, caseData, relatedCases);
  }
...
function createScamTypeSelfAnalysisSection() {
  return `<section class="article-block scam-type-self-analysis" id="scam-type-self-analysis">
  <h2>사기 유형 자가 분석</h2>
  <p>최근 발생하는 투자사기와 각종 금융사기는 업체명과 프로젝트명은 계속 변경되지만 실제 범행 구조는 몇 가지 유형으로 반복되는 특징이 있습니다. 아래에서 어떤 유형에 해당하는지 확인해 보시기 바랍니다.</p>
  <h3>① 주식 리딩방·투자 프로젝트 사칭형</h3>
  <p>가장 많이 확인되는 유형입니다. 범죄 조직은 <strong>교수, 대표, 증권사 이사, 자산운용사 관계자, 애널리스트, 투자 전문가</strong> 등을 사칭하며 접근한 뒤 무료 종목 추천, VIP 투자방, 공모주 특별배정, 기관계좌 운용, AI 투자 시스템, 고수익 프로젝트 등을 제안합니다.</p>
  <p>초기에는 소액 수익을 지급하거나 허위 수익 인증 화면을 보여주며 신뢰를 형성한 뒤 투자금을 점차 늘리도록 유도합니다. 최근에는 '○기 수익 프로젝트', '엘리트 프로젝트', '원자 프로젝트', '아이스버그 프로젝트' 등 프로젝트 형식의 명칭을 사용하는 사례도 반복적으로 확인되고 있습니다.</p>
  <h3>② 가상자산 거래소·코인 투자 사칭형</h3>
  <p>이 유형 역시 <strong>교수, 대표, 자산운용사 관계자, 애널리스트, 투자 전문가</strong> 등을 사칭하여 신뢰를 형성한 뒤 특정 거래소 가입이나 전용 애플리케이션 설치를 권유하는 방식입니다.</p>
  <p>신규 코인 상장, 프라이빗 세일, 해외 거래소, 스테이킹, AI 자동매매 등을 홍보하며 참여를 유도하고, 거래 화면에서는 지속적으로 수익이 발생하는 것처럼 안내합니다. 그러나 실제 출금을 요청하면 각종 이유를 들어 추가 절차를 요구하는 사례가 반복적으로 확인되고 있습니다.</p>
  <h3>③ 팀미션(쇼핑몰 사칭 구매대행·공동구매)·부업·영상 시청 사칭형</h3>
  <p>쇼핑몰 구매대행, 공동구매, 상품 리뷰, 체험단 활동, 영상 시청, SNS 홍보, 좋아요 클릭 등의 간단한 미션을 수행하면 수익을 지급한다고 안내하는 방식입니다.</p>
  <p>처음에는 실제 소액 정산을 진행하여 신뢰를 형성하지만, 일정 단계 이후에는 고액 미션이나 연속 미션을 수행해야 한다며 추가 결제를 요구합니다. 특히 쇼핑몰 운영, 구매대행, 공동구매, 판매 실적 달성 등을 이유로 지속적인 입금을 요구한 뒤 환급이나 정산이 이루어지지 않는 사례가 반복적으로 확인되고 있습니다.</p>
  <h3>④ 라이브 방송·만남 플랫폼 사칭형</h3>
  <p>라이브 방송 플랫폼이나 채팅 애플리케이션을 통해 친분을 형성한 뒤 포인트 충전, 환전, 선물하기, 등급 승급 등을 이유로 입금을 요구하는 유형입니다.</p>
  <p>일정 금액 이상을 충전해야 환전이 가능하다거나 VIP 회원으로 승급해야 출금이 가능하다고 안내하며 지속적인 송금을 유도하는 사례가 확인되고 있습니다.</p>
  <h3>⑤ 환불·보상금 지급 사칭형</h3>
  <p>최근 빠르게 증가하는 유형으로, <strong>로또 추천 사이트, 스포츠토토 정보 제공 사이트, 온라인 게임, 도박 사이트, 투자 사이트 등에 가입했던 이용자에게 개인정보 유출, 서비스 종료, 환불 대상 선정, 보상금 지급 대상이 되었다고 접근</strong>하는 방식입니다.</p>
  <p>처음에는 환불금이나 보상금을 지급해 주겠다고 안내하지만, 환급 절차를 위해 특정 게임 사이트나 가상자산 거래소 가입을 유도하거나 별도의 인증 절차가 필요하다고 설명합니다. 이후 환불금을 받기 위해 충전금, 인증비, 수수료, 보증금 등을 먼저 납부해야 한다며 지속적인 송금을 요구하는 사례가 반복적으로 확인되고 있습니다.</p>
  <h3>공통적으로 나타나는 마지막 단계</h3>
  <p>유형은 서로 다르지만 대부분의 사건은 마지막 단계에서 매우 유사한 모습을 보입니다.</p>
  <p>수익금이나 환불금이 지급되는 것처럼 안내한 뒤 실제 출금이나 환불을 요청하면 <strong>세금, 보증금, 인증비, 전산 처리 비용, 계좌 활성화 비용, 유동성 확보 비용, 출금 승인 비용</strong> 등의 명목으로 추가 입금을 요구하는 사례가 반복적으로 확인되고 있습니다.</p>
  <p><strong>출금거부 또는 출금중지 이후 환불팀, 환불담당자, 고객센터 등을 사칭하며 오히려 추가 입금을 요구받고 있다면 이미 의심 단계를 넘어선 상황에 있습니다. 더 이상의 송금은 바로 멈추고, 현재 피해 상황을 객관적으로 정리하고 대비하는 것이 중요합니다.</strong></p>
</section>`;
}
