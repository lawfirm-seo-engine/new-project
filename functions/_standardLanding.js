...
export function standardPageTitle(caseName = "") {
  return `${standardCaseKeyword(caseName)} 출금불가, 형사고소 진행과 피해 회복 대응 방안`;
}

export function standardMetaDescription(caseName = "") {
  const keyword = standardCaseKeyword(caseName);
  return `${keyword} 피해가 의심된다면 추가 입금을 즉시 중단하고 송금 경위, 대화 기록, 계좌 정보, 사이트 주소, 담당자 프로필 등 관련 증거를 확보하는 것이 중요합니다. 이 페이지에서는 ${keyword}의 주요 수법과 실제 피해 사례, 사기 유형 자가 분석, 출금 거부 및 추가 입금 요구 패턴, 형사고소 절차, 민사 대응, 피해금 회수 가능성 및 초기 대응 방법까지 실제 상담 사례를 바탕으로 자세히 안내합니다.`;
}
...
export function standardHeroText(typeKey = "") {
  const copy = {
    "stock-project": "교수, 대표, 증권사 이사, 자산운용사 관계자, 애널리스트, 투자 전문가 등을 사칭하며 접근한 뒤 무료 종목 추천, VIP 투자방, 공모주 특별배정, 기관계좌 운용, AI 투자 시스템, 고수익 프로젝트 등을 제안받았다면 추가 입금을 즉시 중단하고 점검하기 바랍니다.",
    "institution-exchange": "증권사, 은행, 자산운용사, 가상자산 거래소를 사칭해 신뢰를 얻은 뒤 투자금을 유도하는 유형으로 대표, 증권사 관계자, 자산운용사 매니저, 애널리스트, 투자 전문가 등을 사칭하며 접근하여 VIP방, 공모주, 투자 프로젝트, AI 자동매매, 프라이빗 세일, 해외 선물, 스테이킹 등을 제안받았다면 추가 입금을 즉시 중단하고 점검하기 바랍니다.",
    "team-mission": "쇼핑몰 구매대행, 공동구매, 상품 리뷰, 체험단 활동, 영상 시청, SNS 홍보, 좋아요 클릭 등의 간단한 미션을 수행하면 수익을 지급한다고 안내하는 방식으로 실제 소액 정산을 진행하여 신뢰를 형성하지만, 일정 단계 이후에는 지속적인 입금을 요구하며 환급이나 정산이 이루어지지 않는다면 추가 입금을 즉시 중단하고 점검하기 바랍니다.",
    "live-dating": "채팅 애플리케이션 등에서 친분을 형성한 후 라이브 방송 플랫폼으로 유입시켜 포인트 충전, 환전, 선물하기, 등급 승급 등을 이유로 지속적인 입금을 요구하지만, 일정 금액 이상을 충전해야 환전이 가능하다거나 VIP 회원으로 승급해야 출금이 가능하다며 지속적인 송금을 유도한다면 추가 입금을 즉시 중단하고 점검하기 바랍니다.",
    "refund-reward": "로또 추천 사이트, 스포츠토토 정보 제공 사이트 등에 가입했던 이용자에게 개인정보 유출, 서비스 종료, 환불 대상 선정, 보상금 지급 대상이 되었다고 접근하는 방식으로 환급 절차를 위해 특정 게임 사이트나 가상자산 거래소 가입을 유도하고 환불금을 받기 위해 충전금, 인증비, 수수료, 보증금 등을 먼저 납부해야 한다면 추가 입금을 즉시 중단하고 점검하기 바랍니다.",
  };
  ...
  return [
    {
      ...
      bullets: [
        ...
        "위조 투자 허가서·사업자등록증 등 공문서 형식의 자료를 제공하는 행위는 제347조의2 '컴퓨터등 사용사기죄'에 해당할 수 있습니다.",
        "실제 거래가 없는 허위 어플, 조작된 수익 화면으로 실제 매매 시장으로 인식할 기망행위 등은 통신사기피해환급법 제2조 '전기통신금융사기'와 자본시장법 제8조의2 '무허가 금융투자상품시장 개설죄' 여부까지 경합범으로의 처벌 전략을 수립합니다.",
      ],
    },
    {
      title: "민사 절차 - 형사 절차와 함께 민사 가압류, 지급정지, 손해배상 소송, 부당이득반환 소송 가능성을 동시에 검토해야 회수 경로가 넓어집니다.",
      bullets: [
        "채권 가압류는 상대방이 소송 중 자금을 은닉하지 못하도록 신속히 진행하는 것이 중요합니다.",
        "계좌 명의자들은 피해자에 있어 사기 및 사기 방조, 또는 전자금융거래법 위반에 따른 공동불법행위의 책임이 있습니다.",
        "계좌 명의자들이 사기 조직에 송금의 대가로 부당한 이득을 취하였는지를 확인하여 다양한 소송 경로를 점검해야 피해 회복 가능성과 회수율이 증가합니다.",
      ],
    },
  ];
}
