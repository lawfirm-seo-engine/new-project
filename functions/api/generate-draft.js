...
function createLandingData({ caseName, slug, group, templates = {} }) {
  ...
  const landing = improveGeneratedLanding({
    title,
    description,
    canonical,
    ogTitle: title,
...
function scenarioVictimCases({ scenario, group }) {
  ...
  const cases = {
    app: [
      ...
      `앱 오류를 이유로 재인증비를 요구받았지만 APK 파일명과 알림 기록을 보존해 2차 입금을 중단한 사례`,
    ],
    exchange: [
...
function detectScenario({ caseName = "", slug = "" }) {
  const text = `${slug} ${caseName}`.toLowerCase();
...
function dedupeFaq(faq = []) {
  ...
  for (const item of faq) {
    ...
    const question = sanitizeAwkwardText(item.question);
    const answer = sanitizeAwkwardText(item.answer);
    if (seen.some((existing) => textSimilarity(existing, question) >= 0.72)) continue;
    seen.push(question);
    result.push({ ...item, question, answer });
  }
...
function makeFaq({ caseName, base, group }) {
  ...
  const byType = {
    a: [
      ...
      { question: "고소 후 수사까지 시간이 얼마나 걸리나요?", answer: "사건 복잡성과 관할 경찰서 상황에 따라 다르지만 수사 개시 후 수개월이 일반적입니다. 계좌 추적과 압수수색 시점에 따라 달라지며, 증거가 구체적일수록 절차가 빨라집니다." },
