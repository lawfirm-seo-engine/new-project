...
const TEMPLATE_BODY = [
  ...
  `**형사 고소**: 경찰청 고소장 접수. 계좌 지급정지·동결 요청과 연계해 진행하면 회수 가능성을 높일 수 있습니다.`,
  `**민사 소송**: 상대방 특정이 가능한 경우 가압류·손해배상청구 소송 검토. 수사 결과와 병행해 진행하면 판결 후 강제집행 속도를 높일 수 있습니다.`,
  `**금융당국 신고**: 금융감독원 불법금융신고센터 신고. 불법 플랫폼 차단 및 추가 피해 방지에 기여합니다.`,
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const generatedBody = appendStockReadingroomCta(buildFromTemplate(title, subject, region));
    ...
    if (env.CASES) {
      ...
      if (!batchMode) context.waitUntil?.(upsertCaseInGitHub(env, item, `${existing ? "Update" : "Add"} tujasagi landing ${slug}`).catch(() => {}));
