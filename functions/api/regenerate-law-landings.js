...
function makeBody({ caseName, base, group }) {
  ...
  const typeCopy = {
    ...
    ld: [
      `${displayName} 사건에서 나타나는 리딩방 사칭 사기 패턴은 초대 경로, 수익 인증, 담당자 교체 시점, 출금 거부 패턴을 구조화해 현재 상황과 비교합니다.`,
      `리딩방 사기는 단계별 설계가 치밀합니다. 신뢰 구축 단계에서는 소액 수익을 지급해 실제 서비스처럼 보이고, 전환 단계에서는 고액 유도 후 출금을 차단합니다. 이 구조를 인식하면 2차 입금을 막을 수 있습니다.`,
      `주식리딩방사기에서 주목하는 경고 지표는 출금 신청 후 반복되는 심사 지연, 세금·보증금·인증비 명목의 조건부 요구, 담당자 교체 및 연락 차단입니다.`,
      `증거 보존은 어떤 순서로 해야 하나요? 대화 전체 캡처 → 입금 영수증 저장 → 플랫폼 도메인 기록 → 담당자 프로필 보존 순서로 진행하세요. 앱 설치 파일이 있다면 삭제 전에 저장해 두는 것이 좋습니다.`,
      `금융감독원·수사기관 신고와 상담 접수를 병행하면 어떤 효과가 있나요? 계좌 추적과 형사 절차, 민사 보전 검토를 함께 볼 수 있어 피해회복 가능성을 더 빠르게 확인할 수 있습니다.`,
      `2차 피해는 어떻게 접근하나요? 피해를 당한 뒤 금융피해 전담팀, 법원 판결 대행, 피해금 회수 보장을 제시하며 다시 접근하는 사례가 많습니다. 선입금·수수료 요구가 있다면 반드시 기존 사건과 함께 확인해야 합니다.`,
      `리딩방 피해 구조를 확인한 뒤 증거를 보존한 상태로 상담 접수나 전화 문의로 다음 절차를 잡으세요. 자료가 부족하더라도 현재 남아 있는 증거로 방향 설정이 가능합니다.`,
    ],
    le: [
      ...
      `리딩방 피해회복 안내 페이지에서 사건 구조와 즉시 대응 순서를 확인하세요. 어떤 증거를 어떻게 보존해야 하는지, 어떤 신호가 리딩방 사칭 사기의 전형적인 패턴인지 구체적으로 설명합니다.`,
...
function makeVictimCases({ base, group }) {
  ...
  const extra = {
    ...
    ld: `리딩방 피해회복 검토에서 해당 사건의 자금 흐름이 4단계(접촉→신뢰→고액유도→출금차단) 구조와 일치함이 확인됐습니다. 이 구조를 수사기관 제출 자료에 정리해 피해 패턴 입증에 활용했습니다.`,
...
function makeFaq({ caseName, base, group }) {
  ...
  const byType = {
    ...
    ld: [
      { question: "리딩방 사칭 사기는 무엇을 기준으로 분석하나요?", answer: "유사 리딩방 사건의 초대 경로, 자금 흐름, 출금 거부 패턴, 담당자 교체 시점을 비교 분석합니다. 현재 상황과 일치하는 패턴이 있다면 사기 정황으로 판단할 근거가 됩니다." },
      { question: "리딩방 피해와 단순 투자 실패는 어떻게 구분하나요?", answer: "출금 거부, 추가 입금 요구, 담당자 연락 차단이 있다면 사기 정황입니다. 수익 보장 표현이나 원금 보전 약속도 불법 투자 권유의 지표가 됩니다. 대화 내용을 보존하면 구분이 명확해집니다." },
      ...
      { question: "리딩방 앱이나 HTS/MTS는 어떻게 증거로 남기나요?", answer: "앱 화면을 스크린 녹화로 캡처하고 설치 링크와 APK 파일을 삭제하지 마세요. 스크린샷에 시간 정보가 포함되도록 설정하면 피해 시점 입증에 도움이 됩니다." },
    ],
    le: [
